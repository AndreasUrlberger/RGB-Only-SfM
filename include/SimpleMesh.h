...
class SimpleMesh
{
   public:
     ...
     */
    SimpleMesh(VirtualSensor& sensor, const Matrix4f& cameraPose,
               float edgeThreshold = 0.01f)
    {
        ...
        m_vertices.resize(sensor.GetDepthImageWidth() *
        ...
        for (unsigned int v = 0; v < sensor.GetDepthImageHeight(); ++v)
        {
            ...
            {
                unsigned int idx =
                    v * sensor.GetDepthImageWidth() + u;  // linearized index
                ...
                else
                {
                    ...
                    m_vertices[idx].position =
                        cameraPoseInverse * depthExtrinsicsInv *
                        Vector4f((u - cX) / fovX * depth,
                                 (v - cY) / fovY * depth, depth, 1.0f);

                    // Project position to color map.
                    Vector3f proj = sensor.GetColorIntrinsics() *
                                    (sensor.GetColorExtrinsics() * cameraPose *
                                     m_vertices[idx].position)
                                        .block<3, 1>(0, 0);
                    ...
                    unsigned int idxCol = vCol * sensor.GetColorImageWidth() +
                                          uCol;  // linearized index color
                                                 // unsigned int idxCol = idx;
                                                 // // linearized index color

                    // Write color to vertex.
                    m_vertices[idx].color = Vector4uc(
                        colorMap[4 * idxCol + 0], colorMap[4 * idxCol + 1],
                        colorMap[4 * idxCol + 2], colorMap[4 * idxCol + 3]);
                }
            }
        }

        // Compute triangles (faces).
        m_triangles.reserve((sensor.GetDepthImageHeight() - 1) *
                            (sensor.GetDepthImageWidth() - 1) * 2);
        for (unsigned int i = 0; i < sensor.GetDepthImageHeight() - 1; i++)
        {
            for (unsigned int j = 0; j < sensor.GetDepthImageWidth() - 1; j++)
            {
                ...
                {
                    float d0 =
                        (m_vertices[i0].position - m_vertices[i1].position)
                            .norm();
                    float d1 =
                        (m_vertices[i0].position - m_vertices[i2].position)
                            .norm();
                    float d2 =
                        (m_vertices[i1].position - m_vertices[i2].position)
                            .norm();
                    if (edgeThreshold > d0 && edgeThreshold > d1 &&
                        edgeThreshold > d2)
                        addFace(i0, i1, i2);
                }
                ...
                {
                    float d0 =
                        (m_vertices[i3].position - m_vertices[i1].position)
                            .norm();
                    float d1 =
                        (m_vertices[i3].position - m_vertices[i2].position)
                            .norm();
                    float d2 =
                        (m_vertices[i1].position - m_vertices[i2].position)
                            .norm();
