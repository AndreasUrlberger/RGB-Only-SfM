        ...
        for (int v = 1; v < height - 1; ++v)
        {
            for (int u = 1; u < width - 1; ++u)
            {
                ...
                if (!std::isfinite(du) || !std::isfinite(dv) ||
                    abs(du) > maxDistanceHalved || abs(dv) > maxDistanceHalved)
                {
                ...
                const Vector3f left = pointsTmp[idx - 1];
                const Vector3f right = pointsTmp[idx + 1];
                const Vector3f top = pointsTmp[idx - width];
                const Vector3f bottom = pointsTmp[idx + width];
                const Vector3f normal = (right - left).cross(top - bottom);
